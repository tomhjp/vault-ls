...
import (
	...
	"github.com/tomhjp/vault-ls/internal/uri"
)
...
func TestFilesystem_Change_noChanges(t *testing.T) {
	...
	fh := &testHandler{uri: "file:///test.vault-server"}
	fs.CreateAndOpenDocument(fh, "test", []byte{})
...
func TestFilesystem_GetDocument_success(t *testing.T) {
	...
	dh := &testHandler{uri: "file:///test.vault-server"}
...
func TestFilesystem_ReadFile_memOnly(t *testing.T) {
	fs := NewFilesystem()
	fh := &testHandler{uri: "file:///tmp/test.vault-server"}
...
func TestFilesystem_Open_memOnly(t *testing.T) {
	fs := NewFilesystem()
	tmpDir := TempDir(t)
	testPath := filepath.Join(tmpDir, "test.vault-server")
	fh := testHandlerFromPath(testPath)
...
func TestFilesystem_Create_memOnly(t *testing.T) {
	...
	expectedFis := []string{"test.vault-server"}
...
func TestFilesystem_CreateDocument_missingParentDir(t *testing.T) {
	...
	tmpDir := t.TempDir()
	testPath := filepath.Join(tmpDir, "foo", "bar", "test.vault-server")
	fh := testHandlerFromPath(testPath)
...
func TestFilesystem_HasOpenFiles(t *testing.T) {
	...
	notOpenHandler := filepath.Join(tmpDir, "not-open.vault-server")
	...
	openHandler := filepath.Join(tmpDir, "open.vault-server")
...
func TempDir(t *testing.T) string {
	tmpDir := filepath.Join(os.TempDir(), "vault-ls", t.Name())
